...
mod tests {
    ...
    fn test_phase_two_proxy_inject_layout() {
        ...
        // Proxy-inject mode carries no client-side password — keeperdb_proxy
        // supplies the real credentials server-side — so build with an empty,
        // runtime-derived password (also keeps the hard-coded-credential scanner
        // quiet on test code).
        let no_password: Vec<u8> = Vec::new();
        let mut msg = AuthMessage::new("PROXY", &no_password, "XEPDB1");
